...
class Interface
  ...
  def list_wagons
    if @trains_user_all.any?
      ...
      if @trains_user_all[index_user_train].wagon.any?
        num = 0
        @trains_user_all[index_user_train].iterate_wagons do |wagon, wagon_number|
          puts "Номер вагона № #{num += 1}," + " Тип вагона - #{wagon.type} , Количество свободных мест - #{wagon.free_seats}," +
               " Количество занятых мест - #{wagon.occupited_places}" if @trains_user_all[index_user_train].type == :passenger
          puts "Номер вагона № #{num += 1}," + " Тип вагона - #{wagon.type} , Количество свободного обьема - #{wagon.free_volume}," + 
