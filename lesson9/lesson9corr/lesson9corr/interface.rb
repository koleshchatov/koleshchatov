...
class Interface
  ...
  def create_train
    puts "Выберите тип поезда какой хотите создать:
    1 - Пассажирский
    2 - Грузовой"
    ...
    train_user = case type_train
                 when 1
                   PassengerTrain.new(number: number_train)
                 when 2
                   CargoTrain.new(number: number_train)
                 end
  ...
  def delete_station_rout
    if @routes_user_all.any?
      puts 'Выберите номер маршрута из которого хотите удалить станцию:'
      @routes_user_all.each_with_index do |routes, index|
        print index
        print ': '
        puts routes
  ...
  def add_wagon_train
    puts "Выберите тип вагона какой хотите прицепить:
    1 - Пассажирский
    2 - Грузовой"
  ...
  def delete_wagon_train
    if @trains_user_all.any?
      ...
      if @trains_user_all[index_user_train].wagon.any?
        ...
        if @trains_user_all[index_user_train].speed.zero?
  ...
  def forward_train
    if @trains_user_all.any?
      ...
      if @trains_user_all[index_user_train].next_station.nil?
        ...
        @trains_user_all[index_user_train].next_station.nil?
  ...
  def back_train
    if @trains_user_all.any?
      ...
      if !@trains_user_all[index_user_train].previous_station.nil?
        ...
        @trains_user_all[index_user_train].previous_station.nil?
  ...
  def train_in_station
    if @stations_user_all.any?
      @stations_user_all.each do |station|
        puts "#{station.name} - Поезд №: #{station.trains.map(&:number)}"
      end
    else
      puts 'Нет ни одной станции'
    end
  ...
  def list_wagons
    if @trains_user_all.any?
      ...
      if @trains_user_all[index_user_train].wagon.any?
        num = 0
        @trains_user_all[index_user_train].iterate_wagons do |wagon, _wagon_number|
          if @trains_user_all[index_user_train].type == 'passenger'
            puts " Номер вагона № #{num += 1},"\
                 " Тип вагона - #{wagon.type},"\
                 " Количество свободных мест - #{wagon.free_seats},"\
                 " Количество занятых мест - #{wagon.occupited_places}"
          end
          if @trains_user_all[index_user_train].type == 'cargo'
            puts " Номер вагона № #{num += 1},"\
                 " Тип вагона - #{wagon.type},"\
                 " Количество свободного обьема - #{wagon.free_volume},"\
                 " Количество занятого обьема - #{wagon.all_occupited_volume}"
          end
        end
      else
